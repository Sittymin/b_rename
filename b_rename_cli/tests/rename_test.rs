...
fn test_files_rename_with_move() -> io::Result<()> {
    ...
    let mut input_dir = b_rename_core::dir::InputDir::new(
        base_dir.clone(),
        modify_dir.clone(),
        output_dir.clone(),
        true,
    ...
    assert!(!output_dir.join("modify_file1.ass").exists());
    assert!(!output_dir.join("modify_file2.ass").exists());
    assert!(!output_dir.join("modify_file3.ass").exists());
